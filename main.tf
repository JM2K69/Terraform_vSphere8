...
resource "vsphere_virtual_machine" "NestedESXI" {
    ...
    name             = "${each.key}_${var.DomainName}"
    ...
    ovf_deploy {
        ...
        //local_ovf_path =  "/Storage/VMware/Nested_ESXi8.0_IA_Appliance_Template_v1.ova"
...
resource "vsphere_virtual_machine" "TrueNas" {
    ...
    name                       = "TrueNas_Lab_${var.DomainName}"
    resource_pool_id           = data.vsphere_resource_pool.resource_pool.id
    datastore_id               = data.vsphere_datastore.datastore.id
    datacenter_id              = data.vsphere_datacenter.dc.id
    host_system_id             = data.vsphere_host.host.id
    ...
    ovf_deploy {
        local_ovf_path    = "/Storage/VMware/TrueNas-Appliance-12U7.ova"
        disk_provisioning = "thin"
    }
        lifecycle {
        ignore_changes = all
...
resource "vsphere_virtual_machine" "Client" {
    ...
    count                = var.ClientC
    name                 = "${var.vm-client}_${count.index + 1}_${var.DomainName}"
    folder               = vsphere_folder.parent.path
    resource_pool_id     = data.vsphere_resource_pool.resource_pool.id
    datastore_id         = data.vsphere_datastore.datastore.id
